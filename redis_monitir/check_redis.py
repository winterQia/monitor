...
def check_total(host, port, password):
...
def parse(type, host,port,password):
     rds = Check_Redis(host,port,password)
     if type == 'connections':
...
if __name__ == '__main__':
    try:
        ...
        print "Usage: python %s type 172.16.4.99 6379" % sys.argv[0]
        sys.exit(1)
